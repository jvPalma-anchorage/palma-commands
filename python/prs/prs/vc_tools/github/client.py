...
def list_all_prs():
    ...
    For each author in the config, execute the command:
      gh api -X GET search/issues -f q="repo:{owner}/{repo_name} is:pr is:open author:{author}" 
      -f page=1 -f per_page=5 --jq '.items | .[] | {number: .number, user: .user.login, updated_at: .updated_at}'
    ...
    for author in authors:
        query = f"repo:{owner}/{repo_name} is:pr is:open author:{author}"
        gh_args = [
            "gh", "api", "-X", "GET", "search/issues",
            "-f", f'q={query}',
            "-f", "page=1",
            "-f", "per_page=5",
            "--jq", '.items | .[] | {number: .number, user: .user.login, updated_at: .updated_at}'
        ]
        # Log the command for debugging purposes (note: joining the args is only for display)
        try:
            output = subprocess.check_output(gh_args, text=True)
            # The output may be multiple JSON lines; parse each line
    ...
    # Sort results by updated_at descending (most recent first)
...
def list_pull_request_ids(filters: dict) -> list[tuple[int, str]]:
    """
    Calls list_all_prs to aggregate PRs from all authors and returns a list of tuples (pr_id, "multi").
    """
    data = list_all_prs()
    result = []
    for pr in data:
